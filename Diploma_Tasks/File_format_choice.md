...
После успешной замены файл каждый раз сохраняется. Таким образом можно заменять необходимое количество слов внутри файла формата docx без смен формата и ручного парсинга.
...
45  }
```
В следующем примере был реализован ручной парсинг информации из context.xml, предварительно полученного в результате разархивации файла odt формата.и Функция как в прошлый раз начинает работу с проверки возможности открытия файла, возвращая *false*, если попытка оказалась провальной (5 строка). 
...
Приведённый выше код имеет в себе несколько функций, необходимых для корректного заполнения документа формата docx. Вставленный пример - функция **on_clicked_switchPage**, которая выполняет определённый набор действий:
1. создаёт точную копию оригинального шаблона для упрощения отслеживания работы по замене и автоматизация поставки шаблона для заполнения 
...
2. Инициализация JSON файла для дальнейшей передачи информации скрипту по замене текста (строки 9 - 17)
...
Все программные коды можно просмотреть [другом репозитории](https://github.com/Kalzaver/QtCreatorCodes).